<template>
  <div class="flex h-screen bg-grey-50 dark:bg-grey-950 transition-colors duration-300">
    <!-- Sidebar -->
    <aside :class="[
      'bg-white dark:bg-grey-900 border-r border-grey-200 dark:border-grey-800 transition-all duration-300 flex flex-col',
      sidebarExpanded ? 'w-64' : 'w-20'
    ]">
      <!-- Logo -->
      <div class="h-16 flex items-center justify-between px-4 border-b border-grey-200 dark:border-grey-800 flex-shrink-0">
        <div class="flex items-center gap-3">
          ...
          <span v-show="sidebarExpanded" class="text-lg font-bold text-grey-900 dark:text-grey-50">Digi-School</span>
        </div>
        <button @click="sidebarExpanded = !sidebarExpanded"
          class="p-2 hover:bg-grey-100 dark:hover:bg-grey-800 rounded-lg transition">
          <ChevronLeftIcon  v-if="sidebarExpanded" class="w-5 h-5 text-grey-600 dark:text-grey-300" />
          <ChevronRightIcon v-else                 class="w-5 h-5 text-grey-600 dark:text-grey-300" />
        </button>
      </div>
      ...
      <nav class="flex-1 px-3 py-4 space-y-1 overflow-y-auto custom-scrollbar">
        ...
        <!-- Classes section — driven by classesStore -->
        <div v-if="classesStore.classes.length > 0" class="pt-4">
          <div v-show="sidebarExpanded"
            class="px-3 py-2 text-xs font-semibold text-grey-400 dark:text-grey-500 uppercase tracking-wider">
            Classes
          </div>

          <div v-for="cls in classesStore.classes" :key="cls.id">
            <!-- Class row (click = expand/collapse + navigate) -->
            <button
            ...
            >
              <div class="w-2 h-2 rounded-full flex-shrink-0"
                :style="{ backgroundColor: cls.color || '#3b82f6' }"></div>
              <span v-show="sidebarExpanded"
                class="text-sm font-medium truncate flex-1">{{ cls.name }}</span>
              <ChevronDownIcon
            ...
            <!-- Sub-links -->
            <div
              v-show="sidebarExpanded && expandedClasses.has(cls.id)"
              class="ml-5 mt-0.5 space-y-0.5 border-l-2 border-grey-200 dark:border-grey-800 pl-3"
            >
              <RouterLink
              ...
              >
                <button :class="[
                  ...
                  isActive
                    ...
                    : 'text-grey-600 hover:bg-grey-100 dark:text-grey-400 dark:hover:bg-grey-800'
                ]">
      ...
      <!-- User profile -->
      <div class="border-t border-grey-200 dark:border-grey-800 p-4 flex-shrink-0">
        <Menu as="div" class="relative">
          <MenuButton class="w-full flex items-center gap-3 hover:bg-grey-100 rounded-lg p-2 transition">
            ...
            <div v-show="sidebarExpanded" class="flex-1 text-left min-w-0">
              <p class="text-sm font-medium text-grey-900 dark:text-grey-50 truncate">{{ authStore.user?.name }}</p>
              <p class="text-xs text-grey-600 dark:text-grey-400 truncate">{{ authStore.user?.email }}</p>
            </div>
          </MenuButton>
          <MenuItems class="absolute bottom-full left-0 mb-2 w-48 bg-white dark:bg-grey-900 rounded-lg shadow-lg border border-grey-200 dark:border-grey-800 py-1 z-50">
            <MenuItem v-slot="{ active }">
              <RouterLink to="/settings">
                <button :class="['w-full flex items-center gap-2 px-4 py-2 text-sm text-grey-800 dark:text-grey-200', active ? 'bg-grey-100 dark:bg-grey-800' : '']">
            ...
            <MenuItem v-slot="{ active }">
              <button
                @click="handleLogout"
                :class="['w-full flex items-center gap-2 px-4 py-2 text-sm text-red-600', active ? 'bg-red-50 dark:bg-red-950' : '']"
              >
    ...
    <!-- Main content -->
    <main class="flex-1 overflow-y-auto custom-scrollbar bg-grey-50 dark:bg-grey-950">
      <RouterView />
    </main>
  </div>
</template>
...
const navigation = [
  ...
  { name: 'Mail',               to: '/mail',          icon: EnvelopeIcon },
];
...
onMounted(() => {
  // Load once — classesStore.load() is idempotent (skips if already loaded)
  classesStore.load().then(() => {
    // Auto-expand whichever class is in the current URL
    const match = route.path.match(/^\/class\/(\d+)/);
    if (match) {
      expandedClasses.value = new Set([parseInt(match[1])]);
    }
  });
});
</script>
