  ...
  <div class="h-full overflow-y-auto custom-scrollbar bg-grey-50">
    ...
    <div class="bg-white border-b border-grey-200 px-8 py-6">
      <div class="flex items-center gap-2 text-sm text-grey-500 mb-4">
        ...
        <span>/</span>
        <span class="text-grey-900 font-medium">Insights</span>
      </div>
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-grey-900 mb-1 flex items-center gap-3">
          ...
          <p class="text-grey-500 ml-13">AI-generated analytics from exam corrections</p>
        </div>
        <!-- Scope filter -->
          ...
          <button v-for="s in scopes" :key="s.value"
            ...
            :class="[
              ...
              activeScope === s.value
                ...
                : 'bg-white text-grey-600 border-grey-300 hover:border-indigo-400 hover:text-indigo-600'
            ]">
            {{ s.label }}
          </button>
        </div>
    ...
    <!-- Tab bar -->
    <div class="bg-white border-b border-grey-200 px-8">
    ...
    <div class="p-8">
      ...
      <div v-else-if="activeTab === 'cohort'">
        <div v-if="!cohortInsights.length" class="bg-white rounded-xl border border-grey-200 p-16 text-center">
          <LightBulbIcon class="w-12 h-12 text-grey-300 mx-auto mb-3" />
          <p class="text-grey-500">No cohort insights yet. Grade some exams first.</p>
        </div>

        <div v-else class="space-y-4">
          <!-- Summary row -->
          <div class="grid grid-cols-3 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-grey-200 p-5 shadow-sm">
              <p class="text-2xl font-bold text-grey-900">{{ cohortInsights.length }}</p>
              <p class="text-sm text-grey-500 mt-1">Topics Analysed</p>
            </div>
            <div class="bg-red-50 rounded-xl border border-red-200 p-5 shadow-sm">
              <p class="text-2xl font-bold text-red-600">{{ weakTopics.length }}</p>
              <p class="text-sm text-red-500 mt-1">Topics Needing Remediation</p>
            </div>
            <div class="bg-emerald-50 rounded-xl border border-emerald-200 p-5 shadow-sm">
              <p class="text-2xl font-bold text-emerald-600">{{ strongTopics.length }}</p>
              <p class="text-sm text-emerald-500 mt-1">Topics Well Mastered</p>
            </div>
          </div>
          ...
          <div v-for="t in cohortInsights" :key="t.topic_id"
            ...
            :class="t.weak_student_pct >= 50 ? 'border-red-200' : t.cohort_avg_pct >= 70 ? 'border-emerald-200' : 'border-grey-200'">
            <div class="p-5">
              <div class="flex items-start justify-between gap-4 mb-3">
                <div class="flex-1">
                  ...
                  <h3 class="text-base font-semibold text-grey-900">{{ t.topic_id }}</h3>
                </div>
                <div class="text-right flex-shrink-0">
                  <p class="text-xl font-bold" :class="pctColor(t.cohort_avg_pct)">
                    {{ t.cohort_avg_pct.toFixed(1) }}%
                  </p>
                  <p class="text-xs text-grey-400">Class average</p>
                </div>
              </div>
              ...
              <div class="space-y-2 mb-3">
                <div>
                  <div class="flex justify-between text-xs text-grey-500 mb-1">
                    <span>Class average score</span>
                    <span>{{ t.cohort_avg_pct.toFixed(1) }}%</span>
                  </div>
                  <div class="w-full bg-grey-100 rounded-full h-2">
              ...
              <div v-if="t.recommendation"
                class="mt-3 px-3 py-2 bg-indigo-50 border border-indigo-100 rounded-lg flex items-start gap-2">
                <LightBulbIcon class="w-4 h-4 text-indigo-500 flex-shrink-0 mt-0.5" />
                <p class="text-sm text-indigo-800">{{ t.recommendation }}</p>
              </div>
            </div>

            <div class="px-5 py-2 bg-grey-50 border-t border-grey-100 text-xs text-grey-400">
      ...
      <div v-else-if="activeTab === 'exam'">
        <div v-if="!examTopicSummary.length" class="bg-white rounded-xl border border-grey-200 p-16 text-center">
          <ChartBarIcon class="w-12 h-12 text-grey-300 mx-auto mb-3" />
          <p class="text-grey-500">No exam-topic data yet. Correction insights will appear here.</p>
        </div>
        ...
        <div v-else>
          ...
          <div class="grid grid-cols-2 gap-4 mb-6">
            <div class="bg-white rounded-xl border border-grey-200 p-5 shadow-sm">
              <p class="text-2xl font-bold text-grey-900">{{ examTopicSummary.length }}</p>
              <p class="text-sm text-grey-500 mt-1">Topics with Data</p>
            </div>
            <div class="bg-amber-50 rounded-xl border border-amber-200 p-5 shadow-sm">
              ...
              <p class="text-sm text-amber-500 mt-1">Topics Below 50% Average</p>
            </div>
          </div>
