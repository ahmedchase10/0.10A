  ...
  <div class="h-full overflow-y-auto custom-scrollbar bg-grey-50 text-grey-900 dark:bg-slate-950 dark:text-slate-100">
    ...
    <div class="bg-white border-b border-grey-200 px-8 py-6 dark:bg-slate-900/95 dark:border-slate-800">
      <div class="flex items-center gap-2 text-sm text-grey-500 dark:text-slate-400 mb-4">
        ...
        <span>/</span>
        <span class="text-grey-900 dark:text-slate-100 font-medium">Students</span>
      </div>
      <div class="flex items-center justify-between">
        <div>
          <h1 class="text-3xl font-bold text-grey-900 dark:text-slate-100 mb-1">Students</h1>
          <p class="text-grey-600 dark:text-slate-400">{{ students.length }} student{{ students.length !== 1 ? 's' : '' }} enrolled</p>
        </div>
        <button
    ...
    <div class="p-8">
      ...
      <!-- Empty -->
      <div v-else-if="students.length === 0" class="bg-white rounded-xl border border-grey-200 shadow-sm p-16 text-center dark:bg-slate-900 dark:border-slate-800">
        <UserGroupIcon class="w-14 h-14 text-grey-300 dark:text-slate-500 mx-auto mb-4" />
        <h3 class="text-lg font-medium text-grey-900 dark:text-slate-100 mb-2">No students yet</h3>
        <p class="text-grey-600 dark:text-slate-400 mb-6">Add your first student to get started.</p>
      ...
      <!-- Table -->
      <div v-else class="bg-white rounded-xl border border-grey-200 shadow-sm overflow-hidden dark:bg-slate-900 dark:border-slate-800">
        <table class="w-full text-sm">
          <thead class="bg-grey-50 border-b border-grey-200 dark:bg-slate-900 dark:border-slate-800">
            <tr>
              <th class="px-6 py-3 text-left text-xs font-semibold text-grey-600 uppercase tracking-wider dark:text-slate-400">Student</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-grey-600 uppercase tracking-wider dark:text-slate-400">Student Email</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-grey-600 uppercase tracking-wider dark:text-slate-400">Parent Email</th>
              <th class="px-6 py-3 text-left text-xs font-semibold text-grey-600 uppercase tracking-wider dark:text-slate-400">Added</th>
              <th class="px-6 py-3 text-center text-xs font-semibold text-grey-600 uppercase tracking-wider dark:text-slate-400">Flags</th>
              <th class="px-6 py-3 text-center text-xs font-semibold text-grey-600 uppercase tracking-wider dark:text-slate-400">Actions</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-grey-100 dark:divide-slate-800">
            <tr v-for="student in students" :key="student.id" class="hover:bg-grey-50 dark:hover:bg-slate-800/60 transition">
              <td class="px-6 py-4">
                <div class="flex items-center gap-3">
                  ...
                  <span class="font-medium text-grey-900 dark:text-slate-100">{{ student.name }}</span>
                </div>
              </td>
              <td class="px-6 py-4 text-grey-600 dark:text-slate-300">{{ student.email }}</td>
              <td class="px-6 py-4 text-grey-600 dark:text-slate-300">{{ student.parent_email || '-' }}</td>
              <td class="px-6 py-4 text-grey-500 dark:text-slate-400">{{ formatDate(student.created_at) }}</td>
        ...
        <div class="fixed inset-0 overflow-y-auto">
          <div class="flex min-h-full items-center justify-center p-4">
            <TransitionChild enter="ease-out duration-200" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100"
              ...
              <DialogPanel class="w-full max-w-sm bg-white rounded-2xl shadow-xl dark:bg-slate-900 dark:text-slate-100">
                <div class="p-6 border-b border-grey-200 dark:border-slate-800">
                  <DialogTitle class="text-lg font-semibold text-grey-900 dark:text-slate-100">Add Student</DialogTitle>
                </div>
                <form @submit.prevent="submitAdd" class="p-6 space-y-4">
                  <div>
                    <label class="block text-sm font-medium text-grey-700 dark:text-slate-100 mb-2">Full Name *</label>
                    <input v-model="addForm.name" type="text" required
                      class="w-full px-4 py-2.5 border border-grey-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white text-grey-900 dark:bg-slate-950 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500"
                      placeholder="Ahmed Ben Ali" />
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-grey-700 dark:text-slate-100 mb-2">Email *</label>
                    <input v-model="addForm.email" type="email" required
                      class="w-full px-4 py-2.5 border border-grey-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white text-grey-900 dark:bg-slate-950 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500"
                      placeholder="student@school.tn" />
                  </div>
                  <div>
                    <label class="block text-sm font-medium text-grey-700 dark:text-slate-100 mb-2">Parent Email *</label>
                    <input v-model="addForm.parent_email" type="email" required
                      class="w-full px-4 py-2.5 border border-grey-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white text-grey-900 dark:bg-slate-950 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500"
                      placeholder="parent@family.tn" />
                  </div>
                  <div v-if="addError" class="bg-red-50 border border-red-200 rounded-lg p-3 dark:bg-red-950 dark:border-red-900">
                    <p class="text-sm text-red-700 dark:text-red-300">{{ addError }}</p>
                  </div>
                  <div class="flex gap-3 pt-2">
                    <button type="button" @click="showAddModal = false"
                      class="flex-1 px-4 py-2.5 border border-grey-300 text-grey-700 rounded-lg font-medium hover:bg-grey-50 transition dark:bg-slate-900 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                      Cancel
                    </button>
        ...
        <div class="fixed inset-0 overflow-y-auto">
          <div class="flex min-h-full items-center justify-center p-4">
            <TransitionChild enter="ease-out duration-200" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100"
              ...
              <DialogPanel class="w-full max-w-sm bg-white rounded-2xl shadow-xl dark:bg-slate-900 dark:text-slate-100">
                <div class="p-6 border-b border-grey-200 dark:border-slate-800">
                  <DialogTitle class="text-lg font-semibold text-grey-900 dark:text-slate-100">Edit Student</DialogTitle>
                </div>
                <form @submit.prevent="submitEdit" class="p-6 space-y-4">
                  ...
                  <div v-if="editError" class="bg-red-50 border border-red-200 rounded-lg p-3 dark:bg-red-950 dark:border-red-900">
      ...
      <Dialog @close="showFlagModal = false" class="relative z-50">
        ...
        <div class="fixed inset-0 overflow-y-auto">
          <div class="flex min-h-full items-center justify-center p-4">
            <TransitionChild enter="ease-out duration-200" enter-from="opacity-0 scale-95" enter-to="opacity-100 scale-100"
              leave="ease-in duration-150" leave-from="opacity-100 scale-100" leave-to="opacity-0 scale-95">
              <DialogPanel class="w-full max-w-md bg-white rounded-2xl shadow-xl flex flex-col max-h-[80vh] dark:bg-slate-900 dark:text-slate-100">
                <div class="p-6 border-b border-grey-200 flex-shrink-0 dark:border-slate-800">
                  <DialogTitle class="text-lg font-semibold text-grey-900 dark:text-slate-100">
                ...
                <div class="p-6 overflow-y-auto flex-1 custom-scrollbar">
                  ...
                  <div v-if="studentFlags.length > 0" class="space-y-3 mb-6">
                    <h4 class="text-sm font-medium text-grey-700 dark:text-slate-100">Current Flags</h4>
                    <div v-for="flag in studentFlags" :key="flag.id" class="p-3 bg-orange-50 border border-orange-200 rounded-lg flex justify-between items-start gap-2 dark:bg-orange-950 dark:border-orange-900">
                      <div>
                        <p class="text-sm text-orange-900 break-words dark:text-orange-100">{{ flag.reason }}</p>
                        <p class="text-xs text-orange-600 mt-1 dark:text-orange-300">{{ formatDate(flag.created_at) }}</p>
                      </div>
                      <button @click="removeFlag(flag.id)" class="text-orange-400 hover:text-orange-700 transition flex-shrink-0 p-1 bg-white rounded-md shadow-sm border border-orange-200 dark:bg-slate-900 dark:border-orange-800 dark:text-orange-300">
                        <TrashIcon class="w-4 h-4" />
                      </button>
                    </div>
                  </div>
                  <div v-else class="text-center py-4 mb-4">
                    <p class="text-sm text-grey-500 dark:text-slate-400">No flags for this student.</p>
                  </div>

                  <!-- Add New Flag -->
                  <div>
                    <h4 class="text-sm font-medium text-grey-700 dark:text-slate-100 mb-2">Add New Flag</h4>
                    <textarea v-model="newFlagReason" rows="3"
                      class="w-full px-4 py-2.5 border border-grey-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-orange-500 text-sm bg-white text-grey-900 dark:bg-slate-950 dark:border-slate-700 dark:text-slate-100 dark:placeholder-slate-500"
                      placeholder="Reason for flagging..."></textarea>
                    <div v-if="flagError" class="mt-2 text-sm text-red-600 dark:text-red-300">{{ flagError }}</div>
                  </div>
                </div>

                <div class="p-6 border-t border-grey-200 flex gap-3 flex-shrink-0 bg-grey-50 rounded-b-2xl dark:bg-slate-950 dark:border-slate-800">
                  <button type="button" @click="showFlagModal = false"
                    class="flex-1 px-4 py-2 border border-grey-300 text-grey-700 rounded-lg font-medium hover:bg-grey-100 transition bg-white dark:bg-slate-900 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-slate-800">
                    Done
                  </button>
