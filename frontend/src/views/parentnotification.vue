  ...
  <div class="h-full overflow-y-auto custom-scrollbar bg-grey-50 text-grey-900 dark:bg-slate-950 dark:text-slate-100">
    ...
    <div class="bg-white border-b border-grey-200 px-8 py-6 dark:bg-slate-900/95 dark:border-slate-800">
      <div class="flex items-center gap-2 text-sm text-grey-500 dark:text-slate-400 mb-4">
        ...
        <span>/</span>
        <router-link :to="`/class/${classId}`" class="hover:text-primary-600 dark:hover:text-primary-300 transition">Class</router-link>
        <span>/</span>
        <span class="text-grey-900 dark:text-slate-100 font-medium">Parent Notifications</span>
      </div>
      <h1 class="text-3xl font-bold text-grey-900 dark:text-slate-100">Draft Parent Notification</h1>
      <p class="text-grey-500 dark:text-slate-400 mt-2">Use AI to generate professional, empathetic emails based on student flags.</p>
    </div>
    ...
    <div class="px-8 py-6 grid grid-cols-1 lg:grid-cols-2 gap-6">
      ...
      <div class="space-y-6">
        ...
        <!-- Student Selection -->
        <div class="bg-white rounded-xl border border-grey-200 shadow-sm p-6 dark:bg-slate-900 dark:border-slate-800">
          <h2 class="text-lg font-semibold text-grey-900 dark:text-slate-100 mb-4 flex items-center gap-2">
          ...
          <select 
            ...
            class="w-full px-4 py-2.5 border border-grey-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 bg-white text-grey-900 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
          >
          ...
          <p v-if="selectedStudentId && !selectedStudent?.parent_email" class="mt-3 text-sm text-amber-700 bg-amber-50 border border-amber-200 rounded-lg p-3 dark:text-amber-300 dark:bg-amber-900/20 dark:border-amber-800">
        ...
        <div v-if="selectedStudentId" class="bg-white rounded-xl border border-grey-200 shadow-sm p-6 animate-fade-in dark:bg-slate-900 dark:border-slate-800">
          <h2 class="text-lg font-semibold text-grey-900 dark:text-slate-100 mb-4 flex items-center gap-2">
          ...
          <div v-else-if="flags.length === 0" class="text-sm text-grey-500 bg-grey-50 rounded-lg p-4 dark:text-slate-400 dark:bg-slate-800">
          ...
          <div v-else class="space-y-3">
            <label 
              v-for="flag in flags" 
              :key="flag.id" 
              class="flex items-start gap-3 p-3 rounded-lg border border-grey-200 hover:bg-grey-50 cursor-pointer transition dark:border-slate-700 dark:hover:bg-slate-800"
            >
              <input 
                type="checkbox" 
              ...
              />
              <div>
                <p class="text-sm text-grey-900 dark:text-slate-100">{{ flag.reason }}</p>
                <p class="text-xs text-grey-500 dark:text-slate-400 mt-1">{{ new Date(flag.created_at).toLocaleDateString() }}</p>
              </div>
            </label>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl border border-grey-200 shadow-sm p-6 dark:bg-slate-900 dark:border-slate-800">
          ...
          <textarea
            ...
            class="w-full px-4 py-3 border border-grey-300 bg-white text-grey-900 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 resize-none dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
          ></textarea>
      ...
      <div class="bg-white rounded-xl border border-grey-200 shadow-sm flex flex-col overflow-hidden h-[calc(100vh-12rem)] sticky top-6 dark:bg-slate-900 dark:border-slate-800">
        <div class="p-4 border-b border-grey-200 bg-grey-50 flex items-center justify-between dark:border-slate-800 dark:bg-slate-900">
          <h2 class="text-lg font-semibold text-grey-900 dark:text-slate-100 flex items-center gap-2">
        ...
        <div v-if="!draftEmail" class="flex-1 flex flex-col items-center justify-center p-8 text-center">
          <div class="w-16 h-16 bg-grey-100 rounded-full flex items-center justify-center mb-4 dark:bg-slate-800">
            <DocumentTextIcon class="w-8 h-8 text-grey-400 dark:text-slate-400" />
          </div>
          <h3 class="text-grey-900 dark:text-slate-100 font-medium mb-1">No Draft Yet</h3>
          <p class="text-sm text-grey-500 dark:text-slate-400 max-w-xs">Select a student, provide context, and click "Draft Email" to see the AI-generated message here.</p>
        </div>

        <div v-else class="flex-1 overflow-y-auto p-6 space-y-4">
          <div>
            <label class="block text-sm font-medium text-grey-700 dark:text-slate-300 mb-1">Recipient Email (To)</label>
            <input 
              v-model="recipientEmail" 
              type="email" 
              class="w-full px-4 py-2 border border-grey-300 bg-white text-grey-900 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
            />
            <p class="mt-1 text-xs text-grey-500 dark:text-slate-400">This is filled from the selected student's parent email.</p>
          </div>
          
          <div>
            <label class="block text-sm font-medium text-grey-700 dark:text-slate-300 mb-1">Subject</label>
            <input 
          ...
          <div class="flex-1 flex flex-col">
            <label class="block text-sm font-medium text-grey-700 dark:text-slate-300 mb-1">Body</label>
            <textarea 
              v-model="draftEmail.body" 
              rows="12"
              class="w-full flex-1 px-4 py-3 border border-grey-300 bg-white text-grey-900 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500 resize-none font-sans text-sm dark:border-slate-700 dark:bg-slate-950 dark:text-slate-100"
            ></textarea>
          </div>
        </div>

        <div v-if="draftEmail" class="p-4 border-t border-grey-200 bg-grey-50 flex items-center justify-between dark:border-slate-800 dark:bg-slate-900">
